...
class CountPartitionsWithGivenDifference{
    ...
    static int countPartition(int[] set, int n, int K) {
        int total = 0;
        for(int i=0; i<n; i++){
            total+= set[i];
        }
    ...
    // Time Complexity: O(N*k) + O(N)
    // Space Complexity: O(N*k)
    static int countPartitionTab(int[] set, int n, int K) {
        int total = 0;
        for(int i=0; i<n; i++){
            total+= set[i];
        }
    ...
    static int countPartitionSO(int[] set, int n, int K) {
        int total = 0;
        for(int i=0; i<n; i++){
            total+= set[i];
        }
    ...
    public static void main(String args[]) {
        ...
        System.out.println(countPartitionSO(set, n,K));
    }
}
