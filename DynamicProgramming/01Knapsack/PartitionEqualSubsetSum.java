...
class PartitionEqualSubsetSum {
    ...
    static boolean subsetSumToK(int[] set, int n) {
        int total=0;
        for(int i=0; i<n; i++){
            total+= set[i];
        }
    ...
    // Time Complexity: O(N*k)
    // Space Complexity: O(N*k)
    static boolean subsetSumToKTab(int[] set, int n) {
         int total=0;
        for(int i=0; i<n; i++){
            total+= set[i];
        }
    ...
    static boolean subsetSumToKSO(int[] set, int n) {
         int total=0;
        for(int i=0; i<n; i++){
            total+= set[i];
        }
    ...
    public static void main(String args[]) {
        ...
        System.out.println(subsetSumToK(set, n-1));
    }
}
